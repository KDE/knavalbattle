...
KBattleshipView::KBattleshipView(QWidget *parent, bool draw)
        : KGameCanvasWidget(parent), m_drawGrid(draw)
{
    ...
    m_renderer->resize(32);
    m_renderer->resizeBackground(QSize(10 * 32, 10 * 32));
}
...
void KBattleshipView::startDrawing()
{
    ...
    m_ownField = new KBattleShipField(this, m_renderer, 10);
    m_enemyField = new KBattleShipField(this, m_renderer, 10);
...
void KBattleshipView::adjustFields()
{
    m_ownField->moveTo(10, 10);
    m_enemyField->moveTo(width() / 2 + 10, 10);
}
...
void KBattleshipView::changeEnemyFieldData(int fieldx, int fieldy, int type)
{
    ...
    m_enemyField->setState(fieldx, fieldy, type);
    update();
}
...
bool KBattleshipView::eventFilter(QObject *object, QEvent *event)
{
    if (event->type() == QEvent::KeyPress && m_decide) {
        ...
        QPoint point(mouseEvent->x(), mouseEvent->y());
        if (m_ownField->rect().contains(point)) {
            KBattleShipField::Coord coord = m_ownField->getCoord(point);
            emit sigMouseOverField(coord.first, coord.second);
        }
